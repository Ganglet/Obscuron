...
Scans each panel genome's extracted protein FASTA against Pfam-35 (dark
at T0 = no hit at GA threshold) and Pfam-37 (characterised-by-T1 proxy =
hits a family that's new since Pfam-35), and writes one labeled manifest
row per protein.
...
def main() -> None:
    ...
    with out_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for i, faa_path in enumerate(faa_files, 1):
            ...
            pfam35_hits = scan_genome_against_pfam(faa_path, PFAM35_HMM, cpus=args.cpus)
            pfam37_hits = scan_genome_against_pfam(faa_path, PFAM37_HMM, cpus=args.cpus)
            ...
            if i % 10 == 0 or i == len(faa_files):
                elapsed = time.time() - t0
                print(
                    f"  [{i}/{len(faa_files)}] genomes, {n_total} proteins, "
                    f"{n_dark} dark ({100*n_dark/n_total:.1f}%), {n_positive_proxy} positive-proxy "
